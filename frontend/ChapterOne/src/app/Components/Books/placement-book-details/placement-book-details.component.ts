...
@Component({
  selector: 'app-placement-book-details',
  templateUrl: './placement-book-details.component.html',
  styleUrls: ['./placement-book-details.component.css']
})
export class PlacementBookDetailsComponent implements OnInit {
  theme: string = 'light';
  page: number = 1;
  ...
  ngOnInit(): void {
    ...
    if (!localStorage.getItem('placement')) {
      this.router.navigate(['/Dashboard/PlacementBooks']);
    }
  ...
  onSearch(): void {
    ...
    this.filteredBooks = this.allBooks.filter(book =>
      ...
      (book.bookPlacement?.toLowerCase().includes(term) ?? false)
    );
    this.page = 1;
  }
